...
# ==============================================================================
# --platform=$BUILDPLATFORM: output is platform-independent JS (only dist/
# copies forward), so build natively on the host arch instead of emulating
# the target under QEMU — Bun 1.4 aborts under arm64 QEMU.
...
# not compiled bindings — a dependency's native postinstall must not abort it.
...
ARG APP_VERSION
LABEL org.opencontainers.image.title="transitland-mcp-server"
LABEL org.opencontainers.image.description="Global transit data via the Transitland v2 registry — operators, GTFS/GTFS-RT/GBFS feeds with license terms, routes, stops, and real-time-aware departures via MCP. STDIO or Streamable HTTP."
LABEL org.opencontainers.image.licenses="Apache-2.0"
LABEL org.opencontainers.image.version="${APP_VERSION}"
LABEL org.opencontainers.image.source="https://github.com/cyanheads/transitland-mcp-server"
...
RUN mkdir -p /var/log/transitland-mcp-server && chown -R bun:bun /var/log/transitland-mcp-server
...
ENV LOGS_DIR="/var/log/transitland-mcp-server"
